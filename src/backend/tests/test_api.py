...
def test_health_check(client, mock_pinecone_db):
    ...
    # Mock the 'describe_index_stats' call
    mock_pinecone_db.describe_index_stats.return_value = AsyncMock(total_vector_count=1234)
    
    response = client.get("/health")
    
    assert response.status_code == 200
    assert response.json()["status"] == "healthy"
    assert response.json()["vector_count"] == 1234

@pytest.mark.asyncio
async def test_search_successful(client, mock_pinecone_db):
    """Verify a valid search returns results and correct fields."""
    
    # 1. Setup Mock Data (A high score match)
    mock_pinecone_db.query.return_value = {
        "matches": [
            {
                "id": "scene_001", 
                "score": 0.85, 
...
def test_search_non_ascii_query(client, mock_pinecone_db, mock_encoder):
    ...
    mock_pinecone_db.query.return_value = {"matches": []}
...
def test_pinecone_failure(client, mock_pinecone_db):
    ...
    mock_pinecone_db.query.side_effect = Exception("Connection Timeout")
